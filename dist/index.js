...
function each$1(obj, iter, thisArg) {
    if (isArrayLike(obj)) {
...
function className(obj) {
    ...
    var ret = [];
    for (var key in obj) {
        if (hasOwn$1.call(obj, key) && obj[key]) {
            ret.push(key);
        }
    }
    return ret.join(' ');
...
Parser.prototype = {
    ...
    parse: function parse(source, options) {
        ...
        this.options = extend$1({}, configure(), options);
